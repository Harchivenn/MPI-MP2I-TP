...
// -------- Tableaux redimensionnable ----------

int main(){
    fraction_t frac1 = init_fraction(1,4);
    printf("Initialise : ");
    print_fraction(frac1);
    fraction_t frac2 = init_fraction(1,4);
    printf("Initialise : ");
    print_fraction(frac2);

    fraction_t frac_prod = prod_fraction(frac1, frac2);
    printf("1/4 * 1/4 = ");
    print_fraction(frac_prod);

    fraction_t sum_frac1 = sum_fraction(frac1, frac2);
    printf("1/4 + 1/4 = ");
    print_fraction(sum_frac1);

    fraction_t op_frac = opposite_fraction(sum_frac1);
    printf("(-1)*2/4 = ");
    print_fraction(op_frac);

    fraction_t sub_frac = substract_fraction(frac1, frac2);
    printf("1/2 - 1/2 = ");
    print_fraction(sub_frac);
    printf("------En reference--------\n");
    //Par reference
    fraction_t* frac1_ref = init_fraction_ref(12,6);
    simplify_fraction(frac1_ref);
    fraction_t* frac2_ref = init_fraction_ref(5,0);
    printf("Initialise : ");
    print_fraction_ref(frac1_ref);
    printf("Initialise : ");
    print_fraction_ref(frac2_ref);

    fraction_t* inv_frac = inverse_fraction(frac1_ref);
    printf("1/(0/6) = ");
    print_fraction_ref(inv_frac);

    fraction_t* div_frac = divide_fraction(frac1_ref, frac2_ref);
    simplify_fraction(div_frac);
    printf("(0/6) / (5/0)) = ");
    print_fraction_ref(div_frac);
    ...
    //Ex1.3
    printf("------- Exercice 1.3 ---------\n");
    int_array_t arr1 = init_int_array(6);
    for (int i = 0; i < 6; i++){
        set_value(arr1, i, 6-i);
    }
    print_int_array(arr1);
    printf("valeur a la case 5 dans arr1 = %d\n", get_value(arr1, 5));

    int_array_t* arr1_ref = init_int_array_ref(6);
    print_int_array_ref(arr1_ref);

    int_array_t fibo_arr = init_fibonacci(20);
    print_int_array(fibo_arr);
    
    int_array_t sum_array = sum_int_array(arr1, fibo_arr);
    print_int_array(sum_array);
    
    free_array(sum_array);
    free_array(fibo_arr);
    free_array_ref(arr1_ref);
    free_array(arr1);
    return 0;
}